...
optimize_min_variance <- function(returns_matrix) {
  ...
  # Min variance: w = Σ^(-1)*1 / (1'Σ^(-1)*1)
  cov_inv <- solve(cov_matrix)
  ones <- rep(1, n_assets)
  
  weights <- as.vector(cov_inv %*% ones) / sum(cov_inv %*% ones)
  
  # Apply non-negativity constraint - MUST MATCH frontier
...
compute_efficient_frontier <- function(returns_matrix, risk_free_rate = 0.01, n_points = 50) {
  ...
  frontier <- map_dfr(target_returns_monthly, function(target_ret_monthly) {
    ...
    # Equality constraints: w'μ = target, w'1 = 1
  ...
  # Return both frontier and min variance point
  list(
    frontier = frontier,
    minvar_return = ret_minvar_monthly,
    minvar_volatility = vol_minvar_monthly
  )
}

# Compute frontier
cat("Computing efficient frontier...\n")
frontier_result <- compute_efficient_frontier(returns_df, risk_free_rate = 0.01, n_points = 50)
...
# Create plot
p_frontier <- ggplot() +
  # Efficient frontier (the curved dotted line)
